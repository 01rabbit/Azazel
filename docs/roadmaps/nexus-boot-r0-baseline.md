...
|---|---|---|
| [OF-01](nexus-boot-program-plan.md#15-open-program-findings) — the RAM selector's thresholds contradict the tiers the program assumes | the exit criterion on 8/16/32 GB RAM tiers above, the program plan's R2 selector and R4 Boot exit gate, and the tables those thresholds propagated into in Azazel-Nexus and Azazel-Boot | **resolved 2026-09-19** ([#74](https://github.com/01rabbit/Azazel/issues/74)) by owner decision: boundaries moved to nominal minus a 1024 MiB firmware-reservation allowance. The documentation half is corrected; the selector has no implementation yet, so the correction is written down and unproven |
| [OF-03](nexus-boot-program-plan.md#15-open-program-findings) — the firmware boot path and Secure Boot posture are undeclared | the Nexus and Boot support matrices, Boot's compatibility envelope, and the image-builder decision (Boot ADR-0002) | **resolved 2026-09-19** ([#79](https://github.com/01rabbit/Azazel/issues/79)) by owner decision: booting with Secure Boot enabled via a signed shim chain is the target; disabling it is a fallback requiring the owner's agreement and the recovery key's location; no product key is enrolled into a host. Per-machine state is measured, and nothing has been measured yet |
